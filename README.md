# 糖拌苦力怕入服攻略

> 糖拌苦力怕小百科

## 此文章将全面讲解糖拌苦力怕的各种注意事项及服务器中的游戏设定，建议存为书签

---

## 入服前

认真填写白名单

网址[白名单申请页](https://wj.qq.com/s/1308067/143c)

填写时注意，选择题部分随便填，填空部分请认真填写，填写完后必须加群才能被审核，群 241464054

之后你就会收到qq机器人给你发来的通知，还有你的初始密码

---

## 客户端类问题

客户端请使用官方的客户端版本，已整合高清修复与光影引擎、自带数个光影模型、自带动态环绕mod、物品管理JEI、小地图

不限速直链下载：

[点我下载客户端，1.12.2；mod为拔刀剑、匠魂、暮色森林、植物魔法、料理工艺、更好的动物](https://1dv.papapoi.com/%E7%B3%96%E6%8B%8C%E8%8B%A6%E5%8A%9B%E6%80%95-mod%E5%BD%92%E6%9D%A5v10.1%E6%AD%A3%E5%BC%8F%E7%89%88%281.12.2%29.7z)

推荐使用多线程下载工具下载该文件

> 如果打开提示“未能下载游戏依赖”，那么请单独下载libraries文件夹的内容，解压放到.minecraft文件夹中：
[下载libraries文件夹的内容](https://1dv.papapoi.com/%E8%BD%AF%E4%BB%B6/%E5%88%86%E4%BA%AB%E7%94%A8/libraries.7z)

---

### 客户端运行优化建议

> 更改此设置可以减少将近45%的内存占用

仅对于64位的windows用户（我相信32位电脑优化了也照样卡），第一次游戏时需要检查一项设置，以便流畅游戏

打开启动器HMCL-3.2.112

点击左侧“游戏列表”

点击上方“全局游戏设置”

查看第一项“Java路径”

拉到最右边，点击倒三角展开“Java路径”这一项

如果设置内容为“自定义”，且路径为“jdk8u202-b08-jre\bin\javaw.exe”，则没有错误，可直接运行

如果设置不为自定义，请按上面那一行改一下设置

![启动界面.jpg](/启动界面.jpg)

---

## java下载链接

[https://www.java.com/zh_CN/download/](https://www.java.com/zh_CN/download/)

---

## 服务器地址

```log
自动选择线路：
mc.sweetcreeper.com
电信：
dxmc.sweetcreeper.com
dx.sweetcreeper.com:31072
联通/网通：
wtmc.sweetcreeper.com
wt.sweetcreeper.com:31072
铁通/移动/教育网：
tt.sweetcreeper.com:31072
```

---

## 服务器常识性问题

### 服务器建造常识

1. 建筑请不要在他人未允许的情况下距离他人建筑过近

2. 主城附近的沙子请勿开挖

3. 建筑师申请请参见建筑师申请要求

4. 创造界只有建筑师才能建造，普通玩家仅有参观权限，并可用 / fly 指令开启飞行模式

5. 建筑师请勿在建造界 pvp 及利用 bug 刷物品或状态，严重者撤销建筑师权限

### 会被封禁的行为

1. 盗窃他人物品及方块 5 个以上的

2. 多次未经他人允许，恶意杀人的

3. 屠杀他人农村动物的

4. 利用宠物进行上述行为的

5. 破坏主城及展览馆壁画的

6. 利用 bug 刷物品及金钱的

7. 做出有违服务器团结的行为的

8. 其他应被封禁的行为

### 以下为补充条例

```log
服务器基本条例：
请尊重腐竹，对于服主或者事件处理不满的，请上在心里憋着。不要在游戏中进行纠缠。
请勿向服主索求非法的帮助（刷物品，WE，传送，代圈领地等影响公平性的帮助）
当服主调查服务器事务时，请积极配合。
请您保管好私人财物，箱子请自觉上锁，以防滋长小学生盗窃行为。
请您处理好与周围玩家的关系，保证服务器内的和谐。
与其他建筑间请保持合理的距离。
请勿建造大型或高频率脉冲的红石电路。
请勿建造高空流动水。
请勿建造刷怪塔。
推荐玩家在玩家聚集地中安家，玩家聚集地中通常有完善的交通和良好的起始发展条件。
若您在服务器内遇到难以解决的问题，请联系服主。

以下行为将被处予三年以上、七年以下的封禁处罚：
使用任何方式对其他玩家进行侮辱、诽谤和诬告陷害的。
建造威胁其他玩家财产安全或生命安全的事物，情节严重的。
在服务器内破坏他人建筑物的。
在服务器内破坏公共设施的。
在服务器内盗窃他人财物的。
在服务器内未经对方同意情况下，故意杀害或多次伤害他人，并且态度恶劣的。
其他造成危害服务器秩序的或有背于正常管理的事情，情节严重的。

如有未说明事项，以服主决定为准。
```

---

## 服务器常用地标

> 不定时更新

- 创造界`/warp create`

- 主城`/spawn`或`/warp zhucheng`

- 旧世界（仅限参观）`/warp old`

---

## 宣传 / 捐款

服务器账目 / 捐款方式：

[https://www.chenxublog.com/2015/02/26/%E7%B3%96%E6%8B%8C%E8%8B%A6%E5%8A%9B%E6%80%95%E6%9C%8D%E5%8A%A1%E5%99%A8%E8%B4%A6%E7%9B%AE.html](https://www.chenxublog.com/2015/02/26/%E7%B3%96%E6%8B%8C%E8%8B%A6%E5%8A%9B%E6%80%95%E6%9C%8D%E5%8A%A1%E5%99%A8%E8%B4%A6%E7%9B%AE.html)

服务器宣传网站：

[http://www.sweetcreeper.com](http://www.sweetcreeper.com)

（请勿在别的服务器群或者游戏中宣传，影响他人游戏体验）

---

## 常用指令

|指令|解释|
|:-|:-|
|/register `<密码>` `<再次输入密码>`|进行注册|
|/login `<密码>`|登陆服务器|
|/afk|切换离开状态|
|/back|回到上一个传送地点|
|/me|发一条高亮消息|
|/time|查看时间|
|/home `别名`|回家|
|/home set|设置家|
|/mail clear|清除邮箱|
|/mail send|发信|
|/helpop|联系op寻求帮助|
|/message|私聊某人|
|/reply|回复私聊|
|/suicide|上天|
|/itemsell|卖掉手里的东西，五毛一个|
|/itemsellall|把背包里的东西全买了|
|/tpa `<玩家名>`|请求传送到某人处|
|/tpaccept|接收传送请求|
|/tpdeny|拒绝传送请求|
|/spawn|回到重生点|
|/warp `<地名>`|到指定传送点|
|/warp list|查看所有公共传送点|

---

## 全服通用市场

> 注意，买卖需要缴纳2%的税款

|指令|解释|
|:-|:-|
|/um|打开UI界面|
|/um a `[价格]` `[数量]`|挂售手中的物品|

![um](/um.png)

---

## 金币相关命令

|指令|解释|
|:-|:-|
|/pay `<玩家>` `<金额>`|给指定玩家转钱|
|/baltop `[页码]`|查看金币排行榜|
|/money|查看自己有多少钱|

---

## GiftCode兑换码

> 通常本命令只有激活白名单的时候用，但可能会有其他用处

|指令|解释|
|:-|:-|
|/giftcode use `<激活码>`|兑换东西|

---

## 箱子锁

> 只能锁箱子、木牌、熔炉等物品，其他mod存储设备请自行确保安全

|指令|解释|
|:-|:-|
|/lock|左键锁上方块|
|/unlock|左键方块解锁|
|/keys add `<玩家>`|左键方块给予玩家权限|
|/keys del `<玩家>`|左键方块去除玩家权限|

---

## 圈地

- 圈地不使用金钱，但是会使用你的`领地方块余额`
- 领地方块余额靠挂机获得，也可以自行购买（待验证）
- 领地方块的消耗只取决于x、z轴的大小，不取决于高度
- 圈地使用金铲子右击
- 查看领地信息是用木棍

|指令|解释|命令简写|
|:-|:-|:-|
|/AbandonClaim|删除你所在的领地||
|/AbandonAllClaims|删除你名下的所有领地||
|/ClaimExplosions|关闭领地防爆||
|/AccessTrust|给予另一个玩家低级领地权限（使用门，拉杆之类的）|/at|
|/ContainerTrust|给予玩家中级领地权限（使用箱子，附魔台，击杀动物之类的）|/ct|
|/Trust|给予另一个玩家最高领地权限的指令（可以破坏你的领地）|/t|
|/UnTrust|站在领地内撤销单个玩家的权限|/ut|
|/Untrust  All|撤销这个领地所有玩家的权限||
|/TrustList|列出你所在领地给予了哪些玩家权限||
|/SubdivideClaims|切换铲子的子领地模式，用于划分子领地|/sc|
|/BasicClaims|将铲子切换普通模式|/bc|
|/PermissionTrust|授予玩家与他人共享权限的权限（你给了权限的人，他还可以将权限给别人）|/pt|
|/BuyClaimBlocks|向服务器购买领地方块的指令|/BuyClaim|
|/SellClaimBlocks|出售自己领地方块给服务器的指令|/SellClaim|
|/GivePet|转让宠物的指令（这个插件与香草？联动了的，没装这个的话，忽视）|无|
|/ClaimsList|列出你的领地和所在方位|无|
|/IgnorePlayer|忽略目标玩家的聊天信息（拉黑）|/Ignore|
|/UnIgnorePlayer|取消拉黑这个玩家|/UnIgnore|
|/IgnoredPlayerList|列出你拉黑的玩家（打开黑名单）|/IgnoreList|
|/Siege|宣战（战争服务器玩家之间可以宣战，默认这个功能禁用）|无|
|/Trapped|在别人的领地卡了的指令（使用后倒计时10秒将你从玩家领地传送出去）|无|
|/UnlockDrops|打开死亡掉落（不输入这个指令的话，你死了掉落的东西玩家没法拾取）|无|
|/claimsetspawn|设置领地传送点：站在领地里面输入，你所站的地方就是领地传送点|/cl w|
|/claimspawn|回到领地传送点（前提你设置了的）|/cl w|

---

## 牌子商店

> 货币交易型

- 放置一个箱子和一个牌子（两者任意远）
- 向箱子里放上待交易的商品（一个）
  - `<注：这里需要放置的商品数量是对应牌子价格的单价，如果你放满商品，购买者会以牌子上的单价一次性卖光所以商品>`
- 将牌子修改为指定格式
  - 第一行：商店类型
    - [Sell]  -  卖出型商店
      [iSell]  -  无限卖出型商店
      [Buy]  -  购卖型商店
      [iBuy]  -  无限购买型商店
  - 第二行：任意文字(例：物品名字)
  - 第三行：任意文字(例：介绍|价格）
  - 第四行：单价
- 手持`<红石粉末>`左键箱子 后 持`<红石粉末>`再点击设置好的木牌
- 交易型商店请在创建完成后打开箱子补货

|指令|解释|
|:-|:-|
|/cs hide|隐藏你的销售提醒|
|/cs report|显示自己的销售报告|
|/cs report `<玩家>`|显示指定玩家的销售报告|

---

来自**糖拌苦力怕管理组**

[糖拌苦力怕sweetcreeper.com](https://www.sweetcreeper.com)
